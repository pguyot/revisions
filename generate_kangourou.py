...
function handleKAnswer(choice) {
  const wasFirstAnswer = kAnswers[kIndex] === null;
  ...
  updateKNav();
  // Auto-advance to next unanswered question (only on first answer)
  saveKTime();
  if (wasFirstAnswer) {
...
function confirmStopKangourou() {
  ...
  // Hide stats details during pause — only show progress
...
function showStats() {
  ...
  if (mistakes.length > 0) {
    ...
    for (const m of mistakes) {
      ...
      h += '<tr>' +
        '<td><a href="#" class="overlay-link" data-image="' + q.image + '" data-label="' + label + '">' + label + '</a></td>' +
...
function showQuestionOverlay(imageSrc, label, skipPush) {
  ...
  if (!skipPush) {
    ...
    history.pushState({overlay: true, image: imageSrc, label: label}, '');
  }
}
...
window.addEventListener('popstate', function(e) {
  if (e.state && e.state.overlay) {
    showQuestionOverlay(e.state.image, e.state.label, true);
  } else if (document.getElementById('question-overlay').classList.contains('active')) {
...
document.addEventListener('click', function(e) {
  ...
  if (link) {
    e.preventDefault();
    showQuestionOverlay(link.dataset.image, link.dataset.label);
  }
});
</script>
